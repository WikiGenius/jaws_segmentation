...
def train_net(net,
    ...
    last_num = nums[-1]
    if os.path.isdir(f'./checkpoints/v{last_num}/{plane}'):
        last_num += 1
    
    dir_checkpoint = Path(f'./checkpoints/v{last_num}/{plane}')
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, 'max', patience=1)  # goal: maximize Dice score
...
def train_plane(plane, train_loaders, validation_loaders, n_train_dict, n_validation_dict, experiment, img_size, n_classes=3, save_checkpoint=True, train=True, load_last_model=False, load_interrupted_model=True, bilinear=False, epochs=1, batch_size=16, learning_rate=1e-5, val_percent=0.1, amp=False):
    '''
    plane: str = 'axial' | 'coronal' | sagittal
    '''
    if train:
        try:
            ...
            dir_checkpoint = Path(
                f'./checkpoints/v{last_num_check_folders}/{plane}')
